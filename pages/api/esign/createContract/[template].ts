...
export default async function handler(
  ...
  switch (method) {
    case 'PUT':
      ...
      try {
        if (saved_contract['ok']) {
          ...
          if (process.env.SEND_SMS) {
            await sendSMS(
              "Barvet",
              "Hola! Tienes un contrato por firmar " + `${process.env.API}/sign/${contract["_id"]?.replace("contract:", "")}?token=${contract['token']}`,
              `${body['sendData']['phone']}`
            )
          }
        }
      ...
      res.status(200).json({
        status: 'ok',
        signURL: `${process.env.API}/sign/${contract["_id"]?.replace("contract:", "")}?token=${contract['token']}`,
