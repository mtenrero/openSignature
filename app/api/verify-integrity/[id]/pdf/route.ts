...
import { IntegrityReportGenerator } from '@/lib/pdf/integrityReportGenerator'

export const runtime = 'nodejs'

// GET /api/verify-integrity/[id]/pdf - Generate integrity report PDF (PUBLIC - no auth required)
export async function GET(
  ...
  try {
    ...
    try {
      if (signatureRequest.hashData) {
        ...
        hashVerification = {
          ...
          recalculatedHash: recalculatedHash,
          message: 'Verificación parcial: Esta firma se creó con una versión anterior del sistema.'
        }
      }
    } catch (error) {
    ...
    // Generate PDF report
    const reportGenerator = new IntegrityReportGenerator()
    const pdfBuffer = await reportGenerator.generateIntegrityReport(integrityReportData)
    ...
    return new NextResponse(pdfBuffer, {
      ...
      headers: {
        ...
        'Content-Disposition': `attachment; filename="integrity-report-${signatureId}.pdf"`,
        'Cache-Control': 'no-cache, no-store, must-revalidate',
      },
    })

  } catch (error) {
    console.error('Error generating integrity report PDF:', error)
    return NextResponse.json(
      { error: 'Error al generar el informe de integridad' },
      { status: 500 }
    )
  }
}
...
function getRecommendations(hashValid: boolean, auditSealed: boolean, signatureRequest: any): string[] {
  ...
  if (!hashValid) {
    recommendations.push('El hash del documento no coincide. Investigue posibles alteraciones.')
  }
  
  if (!auditSealed) {
    recommendations.push('La auditoría no está sellada. El rastro de auditoría podría ser modificado.')
  }
  
  if (!signatureRequest.contractSnapshot) {
    recommendations.push('No hay snapshot del contrato. Use versiones más recientes del sistema.')
  }
